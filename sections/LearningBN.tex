...
A discrete CDP $P(X | U)$ can be represented as a table:
\begin{itemize}
...
$\theta_{x(i)|u(i)}$. The likelihood for a single CDP $X$ becomes:
\begin{equation*}
...
\begin{equation}*
    \theta_{x|u}^{\max} = \frac{N_{x,u} + \alpha_{x|u}}{\sum_{x} (N_{x,u} + \alpha_{x|u})}
\end{equation}
