...
Ensemble strategies can be categorized into three main types: 
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Extract from it $m$ different dataset $D^{(i)}$ from the original training set $D$, using \textbf{bootstrap resampling};
...
\begin{itemize}
    ...
    \item Reweight examples in $D$ based on their accuracy according to the trained model (incorrectly classified examples get higher weights);
    \item Train $A$ again on the reweighted dataset;
    \item Repeat the procedure for $m$ times;
    \item Combine the learned models into the final model.
\end{itemize}
This procedure is particullarly effective with weak learners.
\defib{Weak learner}{
