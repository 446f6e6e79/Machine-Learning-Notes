...
\]
\begin{itemize}
    \item The term $p(\boldsymbol{x}|\theta)$ can be easily computed since $\theta$ is known;
...
\paragraph{Example: Bayesian estimation for Gaussian distribution}
...
\end{itemize}
The prior distribution $p(\theta)$ is said to be a \textbf{conjugate prior} for the likelihood function $p(x|\theta)$ if the posterior distribution $p(\theta|x)$ is in the same family as the prior distribution $p(\theta)$.}
