...
\begin{itemize}
    ...
    \item There is a \textbf{training set} $\mathcal{D} = \{(x_1, y_1), (x_2, y_2), \ldots, (x_m, y_m)\}$ of $N$ examples,
...
\begin{itemize}
    ...
    \item For any new example $\boldsymbol{x}$, we can compute the posterior probability for each class $y_i$
...
\end{itemize}
At this point the only factor of equation \ref{eq:independence-assumption} that remains unknown is $p(\boldsymbol{x}|y_i, \mathcal{D})$.
...
\[
    \frac{\partial p(\boldsymbol{x}_j|\theta)}{\partial \mu} = \frac{\partial}{\partial \mu}  \sum_{j = 1}^{n} \ln(\frac{1}{\sqrt{2\pi\sigma^2}}) - \frac{(\boldsymbol{x}_j - \mu)^2}{2\sigma^2} 
\]
...
Repating the same process for $\sigma$, we have:
\[
    \frac{\partial p(\boldsymbol{x}_j|\theta)}{\partial \sigma} = \frac{\partial}{\partial \sigma}  \sum_{j = 1}^{n} \ln(\frac{1}{\sqrt{2\pi\sigma^2}}) - \frac{(\boldsymbol{x}_j - \mu)^2}{2\sigma^2} 
\]
\[
    = \sum_{j = 1}^{n} \sqrt{2\pi} \sigma  \frac{1}{\sqrt{2 \pi}}(-\sigma^{-2}) - \frac{(\boldsymbol{x}_j - \mu)^2}{2} (-2\sigma^{-3})
\]
\[
...
\[
    \sum_{j = 1}^{n} -\sigma^2 + (\bold{x}_j - \mu)^2 = 0
\]
\[
    \sigma^2 = \frac{1}{n} \sum_{j = 1}^{n} (\bold{x}_j - \mu)^2
\]
\paragraph{Conclusion:}
By this two derivations, we have found that the parameters that maximize the likelihood 
of the data are the \textbf{sample mean} and the \textbf{sample variance} of the data.
...
\[
    p(\boldsymbol{x}|y_i, \mathcal{D}_i) = \int_{\theta_i} p(\boldsymbol{x},\theta_i|y_i, \mathcal{D}_i) d\theta_i
\]
Since probability of $\boldsymbol{x}$ given each class $y_i$ is indipendent of the other classes, we can simplify the equation as:
\[
...
\\The prior distribution for $\mu$ is also a Gaussian distribution: $p(\mu) \sim \mathcal{N}(\mu_0, \sigma_0^2)$.
Where:
\begin{itemize}
...
\]
Since we have already seen that $\frac{1}{p(\mathcal{D})}$ is indipendent of $\mu$ and that the examples are i.i.d we can derive:
\[
...
\begin{itemize}
    \item the \textbf{mean} of the posterior distribution $\mu_n$ is a \textbf{linear combination} of the prior mean $\mu_0$ and the sample mean $\hat{\mu}_n$;
    \item the \textbf{more training examples} $(n)$ are seen, the more the sample mean $\hat{\mu}_n$ influences the posterior mean $\mu_n$;
    \item the more training examples are seen, the \textbf{smaller} the variance $\sigma_n^2$ of the posterior distribution becomes, making the distribution sharper around the mean.
\end{itemize}
...
\defib{Statistic}{Any function on a set of examples $\mathcal{D}$ is called a statistic.}
\defib{Sufficient statistic}{A statistic $\boldsymbol{s} = \phi(\mathcal{D})$ is said to be sufficient for a parameter $\theta$ if:
\[ P(\mathcal{D}|\boldsymbol{s}, \theta) = P(\mathcal{D}|\boldsymbol{s}) \]
If $\theta$ is a random variable, then a \textbf{sufficient statistic} contains all the information about $\theta$ that is present in the data $\mathcal{D}$.
}
...
\begin{figure}[H]
    \defib{Conjugate prior}{Given:
    \begin{itemize}
        \item A likelihood function $p(x|\theta)$;
        \item A prior distribution $p(\theta)$;
    \end{itemize}
...
\begin{comment}
    #TODO:
    - Add example for Baesian estimation 
    - Add example of conjugate prior for Multinomial distribution
\end{comment}